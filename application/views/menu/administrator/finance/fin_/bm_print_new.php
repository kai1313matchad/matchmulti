...
<body>
    ...
    <script>
        ...
        {
            $.ajax({
                url : "<?php echo site_url('administrator/Finance/ajax_pick_bmfull/')?>" + id,
                type: "GET",
                dataType: "JSON",
                success: function(data)
                {
                    $('[name="bank_code"]').text(data.BNK_CODE);
                    $('[name="bank_date"]').text(moment(data.BNK_DATE).locale('id').format('DD-MMM-YYYY'));
                    $('[name="bank_info"]').text(data.BNK_INFO);
                ...
                success: function(data)
                {
                    ...
                    for (var i = 0; i < data.length; i++)
                    {
                      var $tr = $('<tr>').append(
                            ...
                            $('<td class="text-center text-uppercase">'+data[i]["COA_ACCNAME"]+' - '+data[i]["BNKDET_INFO"]+'</td>'),
                            $('<td class="text-right chgnum">'+data[i]["BNKDET_AMOUNT"]+'</td>')
                            ).appendTo('#tb_content');
                    };
        ...
        function pick_bmtrxdet(id)
        {
            $.ajax({
                ...
                success: function(data)
                {
                    detail = data.BNKTRX_NUM+' - '+moment(data.BNKTRX_DATE).locale('id').format('DD-MMM-YYYY')+' - '+money_conv(data.BNKTRX_AMOUNT);
